...
namespace CSCoreWaveform
{
    ...
    public partial class MainWindow : INotifyPropertyChanged
    {
        ...
        {
            OpenFileDialog ofn = new OpenFileDialog { Filter = CodecFactory.SupportedFilesFilterEn };
            if (ofn.ShowDialog() == true)
            {
                ...
                IWaveSource source = CodecFactory.Instance.GetCodec(ofn.FileName);
        ...
        private async Task LoadWaveformsAsync(IWaveSource waveSource)
        {
            ...
            float[][] channelData = await WaveformData.GetData(waveSource);
        ...
        private void UpdatePosition()
        {
            ...
            {
                float x = (float)_sampleSource.Position / WaveformData.Length;
                foreach (WaveformDataModel waveformData in Channels)
                {
        ...
        private void Waveform_OnPositionChanged(object sender, PositionChangedEventArgs e)
        {
            if (_notificationSource != null)
            {
                long position = (long)(e.Percentage * WaveformData.Length);
