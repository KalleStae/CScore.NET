...
namespace FadingSample
{
    class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            using (WasapiOut soundOut = new WasapiOut() { Latency = 50 })
            {
                using (FadeInOut fadeInOut = CodecFactory.Instance.GetCodec(openFileDialog.FileName)
                    .ToSampleSource()
                    .AppendSource(x => new FadeInOut(x)))
                {
                    LinearFadeStrategy linearFadeStrategy = new LinearFadeStrategy();
