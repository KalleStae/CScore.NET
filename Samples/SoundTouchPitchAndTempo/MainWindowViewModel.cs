...
namespace SoundTouchPitchAndTempo
{
    ...
    public class MainWindowViewModel : PropertyChangedBase, IMainWindowViewModel
    {
        ...
        {
            string fileName = OpenFileDialog("MP3 Files|*.mp3");
        ...
        {
            double percent = value / (double)maximum;
        ...
        {
            TimeSpan total = _soundOut.WaveSource.GetLength();
            TimeSpan current = _soundTouchSource.GetPosition();

            if (!_stopPositionSliderUpdate)
            {
                double percent = total != TimeSpan.Zero
