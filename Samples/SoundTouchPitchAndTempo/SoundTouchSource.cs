        ...
        public override int Read(float[] buffer, int offset, int count)
        {
            lock (lockObject)
            {
                ...
                while (samplesRead < count)
                {
                    ...
                    {
                        int readFromSource = _sampleSource.Read(_sourceReadBuffer, 0, _sourceReadBuffer.Length);
                        if (readFromSource == 0)
                        {
                            endOfSource = true;
                            _soundTouch.Flush();
                    ...
                    int desiredSampleFrames = (count - samplesRead) / _sampleSource.WaveFormat.Channels;
                    int received = _soundTouch.ReceiveSamples(_soundTouchReadBuffer, desiredSampleFrames) * _sampleSource.WaveFormat.Channels;
