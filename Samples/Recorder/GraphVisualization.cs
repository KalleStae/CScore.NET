...
namespace Recorder
{
    public class GraphVisualization
    {
        ...
        public void Draw(Graphics graphics, int width, int height)
        {
            ...
            float[] samplesLeft = GetSamplesToDraw(_left, width / pixelsPerSample).ToArray();
            float[] samplesRight = GetSamplesToDraw(_right, width / pixelsPerSample).ToArray();
        ...
        private IEnumerable<float> GetSamplesToDraw(List<float> inputSamples, int numberOfSamplesRequested)
        {
            ...
            int resolution = samples.Length / numberOfSamplesRequested;
            int index = 0;
            float currentMax = 0;
            for (int i = 0; i < samples.Length; i++)
            {
