...
namespace EndpointAudioMeterSample
{
    ...
    public sealed class MainViewModel : NotifyPropertyChangedBase, IDisposable
    {
        ...
        private void UpdateDevices()
        {
            ...
            {
                Devices.Clear();
                foreach (MMDevice device in _deviceEnumerator.EnumAudioEndpoints(DataFlow.All, DeviceState.Active))
                {
                    Devices.Add(device);
                }
            });
    ...
    public sealed class AudioMeterModel : NotifyPropertyChangedBase, IDisposable
    {
        ...
        private void UpdateItems()
        {
            ...
            float[] values = _audioMeterInformation.GetChannelsPeakValues();
