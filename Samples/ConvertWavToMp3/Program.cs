...
namespace ConvertWavToMp3
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                Console.WriteLine("Invalid input. we must have a wav file to convert");
                Console.ReadKey();
                return;
            }

            MFMediaType[] supportedFormats = MediaFoundationEncoder.GetEncoderMediaTypes(AudioSubTypes.MpegLayer3);
            ...
            using (source)
            {
                using (MediaFoundationEncoder encoder = MediaFoundationEncoder.CreateMP3Encoder(source.WaveFormat, "output.mp3"))
                {
