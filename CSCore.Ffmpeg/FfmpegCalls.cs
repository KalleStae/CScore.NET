...
namespace CSCore.Ffmpeg
{
    internal class FfmpegCalls
    {
        ...
        internal static unsafe AVOutputFormat[] GetOutputFormats()
        {
            ...
            AVOutputFormat* format = ffmpeg.av_oformat_next(null);
            while (format != null)
            {
                formats.Add(*format);
        ...
        internal static unsafe AVInputFormat[] GetInputFormats()
        {
            ...
            AVInputFormat* format = ffmpeg.av_iformat_next(null);
            while (format != null)
            {
                formats.Add(*format);
            ...
            AvioReadData readData, AvioWriteData writeData, AvioSeek seek)
        {
            ...
            AVIOContext* avioContext = ffmpeg.avio_alloc_context(
                bufferPtr,
                buffer.BufferSize,
                writeable ? 1 : 0,
                (void*)userData,
        ...
        internal static unsafe string AvStrError(int errorCode)
        {
            ...
            string errorMessage = Marshal.PtrToStringAnsi(new IntPtr(buffer), 500).Trim('\0').Trim();
#if DEBUG
