...
namespace CSCore.Codecs.AIFF
{
    ...
    public class AiffReader : IWaveSource
    {
        ...
        public AiffReader(Stream stream)
        {
            ...
            CommonChunk commonChunk = (CommonChunk)_chunkContainer.Chunks.FirstOrDefault(x => x is CommonChunk);
            if (commonChunk == null)
            {
        ...
        public int Read(byte[] buffer, int offset, int count)
        {
            ...
            int read = _stream.Read(b, 0, count);

            int bps = WaveFormat.BitsPerSample;
            if (bps != 8)
            {
                for (int i = 0; i < read; i += WaveFormat.BytesPerSample)
                {
                    if (bps == 16)
                    {
