    ...
    public class CodecFactory
    {
        ...
        public IWaveSource GetCodec(string filename)
        {
            ...
            if (File.Exists(filename))
            {
                ...
                try
                {
                    foreach (KeyValuePair<object, CodecFactoryEntry> codecEntry in _codecs)
                    {
                        try
                        {
                            if (
        ...
        public IWaveSource GetCodec(Uri uri)
        {
            ...
            try
            {
                string filename = TryFindFilename(uri);
                if (!String.IsNullOrEmpty(filename))
                {
...
#pragma warning restore 618
        ...
        private string TryFindFilename(Uri uri)
        {
            ...
            string path = Win32.NativeMethods.PathCreateFromUrl(uri.OriginalString);
