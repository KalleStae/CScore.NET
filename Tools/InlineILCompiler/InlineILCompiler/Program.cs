...
namespace InlineILCompiler
{
    class Program
    {
        ...
        static string[] GetSourceFilesFromProject(string proj)
        {
            ...
            XmlTextReader rdr = new XmlTextReader(proj);
            while (rdr.Read())
            {
        ...
        static bool PreprocessIL(List<string> ilCode, List<Tuple<string, string, int>> exports, Dictionary<string, List<CodeSegment>> segments)
        {
            ...
            for (int i = 0; i < ilCode.Count; i++)
            {
                ...
                else if (line.StartsWith(".line"))
                {
                    ...
                    {
                        foreach (CodeSegment codeSegment in cs)
                        {
        ...
        {
            foreach (KeyValuePair<string, List<CodeSegment>> kvp in segments)
            {
                foreach (CodeSegment seg in kvp.Value)
                {
                    if (!seg.Inserted)
                    {
        ...
        static int Main(string[] args)
        {
            ...
            try
            {
                ...
                string[] sources = GetSourceFilesFromProject(ProjectPath);
            ...
            {
                string[] files = new[] { Path.Combine(working, "disasm.il"), Path.Combine(working, "disasm.res") };
